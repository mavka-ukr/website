...
const mavkaSyntax = () => {
  return {
    ...
    keywords: {
      ...
      keyword:
        "та композиція видалити перелік риса втілює зовн внут біб імп спец власна ціль макет це дія рівно більше цикл менше макет пак спробувати містить не я js файл впасти тривала зловити є перебрати кінець структура чекати взяти якщо або і інакше вернути поки як дати модуль пропустити зупинити",
      literal:
        "так ні перебір перебір_з_ключем пусто символ текст юнікод логічне число п32 ц32 д32 список словник обʼєкт байти"
    },
    unicodeRegex: true,
    contains: [
      {
...
const tsilSyntax = () => {
  return {
    ...
    keywords: {
      $pattern: /[a-zA-Zа-яА-ЯіІїЇєЄ_ʼ]+/,
      keyword:
        "змінна стала ціль дія зовнішня вернути взяти визначення якщо поки",
